...
class Binance(exchange.Exchange):
    ...
    def symbol_ticker(self):
        ...
        print(response)
        return Price(pair= self.get_symbol(), currency=self.currency.lower(), asset= self.asset.lower(), exchange=self.name.lower(), current=response['price'])
    ...
    def historical_symbol_ticker_candle(self, start: datetime, end=None, interval=Client.KLINE_INTERVAL_1MINUTE):
        #conversion second interval to string like "1m"
        ...
        for candle in self.client.get_historical_klines_generator(self.get_symbol(), interval, start, end):
            output.append(
                Price(pair=self.compute_symbol_pair(), currency= self.currency.lower(), asset = self.asset.lower(), exchange=self.name.lower(),current= candle[1],
                      lowest=candle[3], highest=candle[2], volume=candle[5], openAt=utils.format_date(datetime.fromtimestamp(int(candle[0])/1000)))
            )
        return output
